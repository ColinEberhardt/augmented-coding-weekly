...
author: ceberhardt
published: true
thumbnail: img/40.png
summary: Anthropic's Mythos model represents a massive capability leap with 15-20% improvements on coding benchmarks, though it's so powerful they're restricting its release due to security concerns. Meanwhile, the industry grapples with fundamental questions about LLM development practices and the race intensifies for universal AI agents, with OpenAI launching Codex to compete with Claude Code's success.
---

## [Mythos System Card](https://cdn.sanity.io/files/4zrzovbb/website/7624816413e9b4d2e3ba620c5a5e091b98b190a5.pdf)
...
The latest model from Anthropic is so powerful they haven't formally released it yet. During model evaluation they determined that its ability to find exploits was so significant that they are initially only making it available to select organisations via project [Glasswing](https://www.anthropic.com/glasswing). For a quick overview of this story, I'd recommend [taking a look at Simon Willison's blog post](https://simonwillison.net/2026/Apr/7/project-glasswing/).

![benchmark]({{ "img/40.png" | absolute_url }})

While the model's ability to find zero days (vulnerabilities that no-one has yet discovered) grabbed most of the headlines, as well as the story about it [escaping containment and emailing a research who was eating a sandwich on a park bench](https://medium.com/@william.couturier/the-sandwich-email-8aad9959e6bc), digging into the system card reveals that its capability represents a huge leap forwards.
...
<small>DTRACE.ORG</small>

This is an excellent and highly quotable blog post about the perils of giving LLMs / AI Agents oo much latitude.

Bryan starts by introducing Larry Wall's concept of the _lazy_ developer, who is motivated to minimise effort an energy expenditure. This, mixed with impatience (anticipating future needs and write code against them) and hubris (writing code they are proud of and others will admire), drives lazy developers towards creating elegant abstractions.
...
[Just last week](https://augmentedcoding.dev/issue-39/) I included a link to a GitHub issue that claimed Claude Code was now unusable due to silent changes in behaviour. Here's another one, this time pointing to a configuration change that has a significant cost impact.
